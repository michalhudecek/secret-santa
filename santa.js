...
const TEST_EMAIL_ADDRESS = "michal.hudecek@gmail.com"; // Your email address for testing
...
const SANTAS = [
  ...
  "vera.hudeckova@centrum.cz",
];

// Pairs of emails that should never be assigned to each other
const RESTRICTIONS = [
    ...
    for (let index = 0; index < GIFTS_PER_SANTA; index++) {
      let availableGiftsForThisSanta = gifts[index].filter((email) => {
        if (santa === email) {
          ...
          return false;
        } else if (RESTRICTIONS.some(([restricted1, restricted2]) => (restricted1 === santa && restricted2 === email) || (restricted1 === email && restricted2 === santa))) {
          return false;
        } else {
          return true;
        }
      ...
      let randomGift =
        availableGiftsForThisSanta[
          Math.floor(Math.random() * availableGiftsForThisSanta.length)
        ];
